  ...
  def page_should_have(text_or_regexp)
    if text_or_regexp.class == String
      if response.respond_to? :should
        response.should contain(text_or_regexp)
      else
        assert_contain text_or_regexp
      end
  ...
  def page_should_not_have(text_or_regexp)
    if text_or_regexp.class == String
      if response.respond_to? :should_not
        response.should_not contain(text_or_regexp)
      else
        assert_not_contain(text_or_regexp)
      end
    elsif text_or_regexp.class == Regexp
      if content.respond_to? :should_not
        content.should_not contain(text_or_regexp)
      else
        assert_no_match(text_or_regexp, content)
      end
    else
      raise "Expected String or Regexp, got #{text_or_regexp.class}"
    end
